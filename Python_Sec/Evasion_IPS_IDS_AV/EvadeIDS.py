...
def ddosTest(src, dst, iface, count):
	...
	send(pkt, iface=iface, count=int(count))
...
def main():
	parser = optparse.OptionParser('Usage%prog' + '-i <interface> -s <source> -t <target> -c <count>')
	parser.add_option('-i', dest='iface', type='string', help='specify the interface')
	parser.add_option('-s', dest='src', type='string', help='specify the source')
	parser.add_option('-t', dest='tgt', type='string', help='specify the target')
	parser.add_option('-c', dest='count', type='string', help='specify the count')
